...
def add_prod(request, product_id, flag=False):
    ...
    if request.method == 'POST':
        ...
        for item in request.POST:  
            ...
            if key is not first_key:  # Nos saltamos el primer elemento del DicSet
                ...
                mini_context += f"<p> minicontext POST:{item} | Llave CAT: {key} - valor VAR: {value} - opcion STOCK: {value_stock} </p>"
                dataset.append(item)
            ...
            try:
                ...
                for var in variations: # ciclo para obtener al menos el primer valor, sin filtrar con data-*
                    ...
                    var_context += f"<p> VAR_CONTEXT Producto: {var.product} | Categoria: {key} {var.variation.varcat} | Variacion: {value} {var.variation.variation} ↑ valor: {var.value} </p>"
...
def ecart(request, total=0, quantity=0, cart_items=0):
    try:
        cart = Cart.objects.get(cart_id=_cart_id(request)) # get, solo trae 1 objeto
        cart_items = CartItem.objects.filter(cart=cart, is_active=True)  # filter, trae todos los que encuentre
        sub_total=None
